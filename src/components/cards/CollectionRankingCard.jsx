...
const CollectionRankingCard = ({ id, contractAddress, Logo, Name, Volume, isVerified, Royalty, Floor, Listings, venomProvider }) => {
    ...
    const gettingTotalSupply = async () => {
        if (venomProvider != undefined) {
            try {
                const contract = new venomProvider.Contract(collectionAbi, contractAddress);
    ...
    return (
        <Link href={`/collection/${contractAddress}`} className="flex transition-shadow hover:shadow-lg" role="row">
            <div
            ...
            >
                ...
                <div className="relative mr-2 w-8 shrink-0 self-start lg:mr-5 lg:w-12">
                    <Image src={Logo ? Logo.replace("ipfs://", "https://ipfs.io/ipfs/") : venomLogo} height={100} width={100} alt="collectionlogo" className="rounded-2lg" loading="lazy" />
                    {isVerified ?
            ...
            <div
                ...
                role="cell"
            >
                <span className="dark:text-jacarta-200 text-jacarta-700">{Royalty ? Royalty : "0"} %</span>
            </div>
            ...
            <div
                ...
                role="cell"
            >
                <span className='dark:text-jacarta-200 text-jacarta-700'>{totalSupply ? totalSupply : "0"}</span>
            </div>
        </Link>
    )
}
