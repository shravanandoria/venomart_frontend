...
import {
  get_launchpad_by_name,
  get_user_mints,
  updateLaunchpadStatus,
} from "../../utils/mongo_api/launchpad/launchpad";
...
const launchpad = ({
  ...
  const handlemintCountInc = () => {
    if (mintCount >= selected_phase?.maxMint - phaseMintedCount)
      return alert(`You only have ${selected_phase?.maxMint - phaseMintedCount} mints left in this phase!`);
  ...
  const refreshLatestMints = async () => {
    ...
    const res = await loadNFTs_user(venomProvider, signer_address, undefined, client, "newestFirst");
    let new_nfts = [];
    res?.nfts
    ...
    try {
      const mappingNFTs = await Promise.all(
        new_nfts.map(async nft => {
          let jsonURL = nft?.files[0].source;
          try {
            const JSONReq = await axios.get(jsonURL);
            let attributes = JSONReq.data.attributes;
            const createdNFT = await addNFTViaOnchainLaunchpad(
              nft,
              attributes,
              signer_address,
  ...
  const getPhaseWiseMinted = async () => {
    ...
    const walletMintCount = await get_address_mint_count(
      venomProvider,
      collectionData?.contractAddress,
      selected_phase?.id,
      signer_address,
  ...
  const mintLaunchNFT = async () => {
    ...
    // minting here
    try {
      const launchMint = await launchpad_mint(
        venomProvider,
        collectionData?.contractAddress,
        signer_address,
        mintCount,
  ...
  return (
    <div className={`${theme}`}>
      <Head>
        ...
        <meta
          name="keywords"
          content={`venomart, venom blockchain nft,nfts on venom, NFT launchpad, nft launchpad venom, best nft launchpads, ${
            LaunchData?.name ? LaunchData?.name : ""
          ...
          <section className="relative bg-light-base pb-12 pt-12 dark:bg-jacarta-800">
            <section className="text-gray-600 body-font overflow-hidden">
              ...
              <div className="container px-5 pt-6 pb-24 mx-auto">
                    ...
                    <div className="mt-4 mb-12">
                      ...
                      <div className="flex flex-col w-[100%] mt-6">
                        {collectionData?.phases?.map((phase, index) => (
                          <div
                            className={`${
                              selected_phase?.id == index && "border-2"
                            ...
                            <div className="flex flex-col  w-[70%] text-white">
                              ...
                              <p
                                className={`text-[14px] font-mono ${
                                  theme == "dark" ? "text-[#efefef]" : "text-[#191919]"
                                }`}
                              >
                            ...
                            <div className="flex justify-between w-[100%]">
                              <div>
                                <h2 className={`font-bold ${theme == "dark" ? "text-[#f1f1f1]" : "text-[#363232]"}`}>
                                  <span
                                    className={`${theme == "dark" ? "text-[#efefef]" : "text-[#292929]"} font-light`}
                                  >
                                    Price:
                                  </span>{" "}
                              ...
                              <div className="relative items-center">
                                <div
                                  className={`whitespace-nowrap absolute right-[4px] top-[-26px] text-[15px] font-mono ${
                                    theme == "dark" ? "text-[#efefef]" : "text-[#191919]"
                                  }`}
                                >
                                  ({phaseMintedCount}/{selected_phase?.maxMint}) Minted
                                </div>
                            ...
                            <div className="flex w-[100%] mt-4">
                              {new Date(selected_phase?.startDate) > new Date() ? (
                                ...
                                mintLoading ? (
                                  ...
                                  >
                                    <span className="text-white font-mono">
                                      Mint {phaseMintedCount >= selected_phase?.maxMint && "🔒"}
                                    </span>
                                  </button>
                                )
                              ) : (
                ...
                {userMints == true && (
                  <div
                    className={`dark:bg-jacarta-900 lg:w-4/5 mx-auto flex flex-col w-[100%] my-2 mt-12 p-4 justify-between shadow-md shadow-[#dcdcdc] dark:shadow-[#0D102D] rounded-[13px] `}
                  >
                    <h2 className="text-lg text-jacarta-700 dark:text-white tracking-widest font-bold font-mono">
                      Your Mints 🎉
                    </h2>
                    <p className="text-[16px] text-jacarta-700 dark:text-white tracking-widest font-mono mb-2">
                      <span className="text-blue cursor-pointer" onClick={() => refreshLatestMints()}>
                        Click here
                      </span>{" "}
                      to refresh your latest mints!
                    </p>
                  ...
                  <div className="modal-body p-6">
                    <div className="mb-2 flex items-center justify-between">
                      <span className="font-display text-[18px] font-semibold text-jacarta-700 dark:text-white">
                        You have successfully minted {mintCount} {collectionData?.name} NFT for{" "}
                        {selected_phase?.mintPrice} VENOM <br />{" "}
                        <span className="font-mono text-[16px] font-light text-jacarta-700 dark:text-white">
                          (You can view the minted NFTs on this page and in your profile as well :)
                        </span>
                      </span>
                    </div>
                  </div>
